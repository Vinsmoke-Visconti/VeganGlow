...
const INGREDIENTS = [
  { name: 'Rau Má', desc: 'Chiết xuất từ Centella Asiatica — phục hồi và làm dịu da nhạy cảm.', icon: <Leaf size={24} /> },
  { name: 'Trà Xanh', desc: 'Chống oxy hóa mạnh, bảo vệ da khỏi tia UV và ô nhiễm môi trường.', icon: <Flower2 size={24} /> },
  { name: 'Diếp Cá', desc: 'Kiềm dầu, ngừa mụn tự nhiên — bí quyết làm đẹp dân gian Việt Nam.', icon: <TrendingUp size={24} /> },
  { name: 'Tràm Trà', desc: 'Kháng khuẩn dịu nhẹ, hỗ trợ làn da khỏe mạnh từ sâu bên trong.', icon: <ShieldCheck size={24} /> },
];
...
const TEAM = [
  ...
  },
  {
    ...
    initials: 'HNV',
    bio: 'Kiến trúc sư trưởng của hệ thống, người đảm bảo trải nghiệm mua sắm mượt mà và hiện đại.',
  },
  {
    name: 'Phạm Hoài Thương',
    role: 'Operations Manager',
    initials: 'PHT',
    bio: 'Quản lý vận hành và chuỗi cung ứng, đảm bảo từng sản phẩm được đóng gói hoàn hảo nhất.',
  },
  {
    name: 'Trần Quỳnh Trâm',
    role: 'Technical Lead & Security',
    initials: 'TQT',
    bio: 'Chuyên gia phụ trách hạ tầng kỹ thuật và bảo mật thông tin, bảo vệ dữ liệu khách hàng.',
  },
];
    ...
    <div className={styles.page}>
      ...
      <div className="container">
        ...
        <section className={styles.glassSection}>
          <FadeIn className={styles.sectionHeader}>
            <h2 className={styles.sectionTitle}>
              Thảo dược kim chỉ nam
            </h2>
            <p className={styles.sectionSubtitle}>
              Những nguyên liệu Việt Nam quen thuộc, được khoa học chứng minh hiệu quả tuyệt vời cho làn da
            </p>
          </FadeIn>

          <StaggerContainer className={styles.ingredientsGrid}>
            {INGREDIENTS.map((ing) => (
              <StaggerItem key={ing.name} className={styles.ingredientCard}>
        ...
        {/* CTA Section (2-Column Glass Style) */}
        <section className={styles.ctaSection}>
          <div className={styles.ctaLayout}>
            <FadeIn direction="left" className={styles.ctaImageWrap}>
              <Image 
                src="/images/why-liquid.png" 
                alt="VeganGlow Botanical Liquid" 
                width={600} 
                height={600} 
                className={styles.ctaLiquidImage}
              />
            </FadeIn>

            <FadeIn direction="right" className={styles.ctaInner}>
              <h2 className={styles.ctaTitle}>Bắt đầu hành trình thuần chay</h2>
              ...
              <div className={styles.ctaCheck}>
                <CheckCircle2 size={20} /> Miễn phí giao hàng cho đơn từ 500.000đ
              </div>
            </FadeIn>
          </div>
        </section>
