...
type Props = { searchParams: Promise<{ q?: string; category?: string; status?: string }> };
...
export default async function AdminBlogsPage({ searchParams }: Props) {
  ...
  const { data: blogs, error } = await query as { data: BlogPost[] | null, error: any };
    ...
    <div className={shared.page}>
      <div className={shared.toolbar}>
        <div style={{ display: 'flex', gap: 12, flex: 1 }}>
          <BlogFilters defaults={{ q: sp.q, category: sp.category, status: sp.status }} />
        </div>
